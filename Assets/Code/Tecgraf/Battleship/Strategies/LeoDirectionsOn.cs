...
{
    public class LeoDirections : IBombardmentStrategy
    {
        ...
        private bool firstH = true;
        private bool secH = false;
        private int _firstHitX, _firstHitY;
        private int turn = 0;
        public bool isDebuging = false;

        public LeoDirections()
        {
        ...
        public void Bombard(Board knownBoard, out int x, out int y)
        {
            ...
            if (isDebuging) Debug.Log("Turn " + turn);
            ...
            if (knownBoard.GetCellState(_lastX, _lastY) == BoardCellState.Occupied)
            {
                ...
                else
                {
                    if (secH == true)
                    {
                        dir = FindDirection(_firstHitX, _lastX);
                        for (int i = 0; i < _positions.Count; i++)
                        {
                            if (FindDirection(_firstHitX, (int)_positions[i].x) != dir)
                            {
            ...
            if (knownBoard.GetInfo(_lastX, _lastY) is IShip)
            {
                ...
                _positions.Clear();
                if (isDebuging) Debug.Log("clear _positions");
